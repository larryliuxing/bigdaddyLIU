...
export function AdminLeaderboardPanel({ adminName }: { adminName: string }) {
  ...
  const applyBoard = useCallback((board: { entries?: LeaderboardEntry[]; stats?: LeaderboardStats } | null) => {
    if (!board) return;
  ...
  const load = useCallback(async () => {
    ...
    setPercent(Number(data.thresholdPercent) || DEFAULT_LEADERBOARD_THRESHOLD_PERCENT);
  ...
  async function removeEntry(entry: LeaderboardEntry) {
    ...
    try {
      const res = await fetch(
        `/api/leaderboard?memberId=${entry.memberId}`,
        { method: "DELETE" },
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
